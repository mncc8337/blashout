...
func spawn_foe():
	...
	if foe_class == FOE_CLASS.RANDOM:
		...
		foe_instance.attack_cooldown = foe_attack_cooldown_max * rng.randf_range(1, 1.5)
	elif foe_class == FOE_CLASS.BIGASS:
		foe_instance.attack_dmg = foe_attack_dmg_max * rng.randf_range(0.9, 2)
		foe_instance.SPEED = foe_speed_max * rng.randf_range(0.4, 0.7)
		foe_instance.max_health = foe_health_max * rng.randf_range(1.1, 2)
		foe_instance.attack_cooldown = foe_attack_cooldown_max * rng.randf_range(1, 2)
		...
		foe_instance.attack_cooldown = foe_attack_cooldown_max * rng.randf_range(0.3, 0.5)
